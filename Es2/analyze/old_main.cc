...
int main() {
    ...
    TH1D h_Sig ("h_Sig", "MVA signal", 100, 0., 1.0);
    TH1D h_Bkg ("h_Bkg", "MVA background", 100, 0., 1.0);
    ...
    auto legend = new TLegend(0.7,0.75,0.85,0.85);
    legend->AddEntry(&h_Sig,"Signal","l");
    legend->AddEntry(&h_Bkg,"Background","l");
    h_Sig.SetTitle("MLP discriminant with O_{MLP}>0.5");
    h_Sig.Draw();
    h_Bkg.Draw("SAME");
    legend->Draw("SAME");

    sig_bkg.SaveAs("MLP_cut.pdf");
